...
**Bead:** `tf-0awo.26` · **Instrument:** `packages/runtime/test/unified/cross-attempt-output-flush.test.ts` (a runtime test, **not** a tiny-firegrid public-surface sim — see "Why a runtime test")
...
## Why a runtime test (not a tiny-firegrid sim), and the no-backdoor stance
