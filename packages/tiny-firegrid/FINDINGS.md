...
| TFIND-015 | **B IMPLEMENTED — #350 coordinator-APPROVED at the correctness bar (2026-05-18); merge-gated on full GitHub CI green only.** Verified in-diff: ACP codec drops `WorkflowEngine`/`DurableDeferred` (import line removed, no re-add); workflow owns the bridge via the *existing* `runtimeInputDeferredFor(contextId,seq)` (no new wait primitive); deterministic test has a real blocked-pending point BEFORE response ingress + exact `runtime-context/<contextId>/input/1` deferred + no sleep-race (emit-then-wait bar met); no forcing/widening cast (only `as const`); SDD #350 §7 structural proof; no FINDINGS/tmp hunk. On merge → toy `permission-flow-pipeline` capability EXISTS → next dispatchable toy build (after capability re-check). | permissions / codecs | Permission flow and codec authority remain unsettled. | cat-1/2 production architectural (resolved-direction: B). ACP codec currently does durable-deferred authority inside the codec (verified) vs README "codecs may not own durable permission state" — B aligns impl to the documented contract; TFIND-041 authority-family precedent |
