...
| TFIND-040 | SDD `SDD_FIREGRID_SESSION_OBSERVATION_SURFACE.md` (`sidecar/session-observation`). **Q1 DECIDED A — attach at SESSION level** (Gurdas 2026-05-18; rationale: a context handle would be host-plane leakage up into the session plane — plane-boundary discipline, not just YAGNI). Q2/Q3/Q4 presented to Gurdas w/ coordinator concurrence; impl queued behind keystone. | client-sdk / observations | Client SDK lacks a per-event session observation surface. | cat-1 (rubric anchor — real client consumer pattern: `session.subscribe()` per-event; correctly SDD-first) |
