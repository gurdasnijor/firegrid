...
| TFIND-015 | **RESOLVED — B implemented + merged (#350 `5ecc20d53`, full CI green, coordinator-approved at the correctness bar).** ACP codec dropped `WorkflowEngine`/`DurableDeferred`; workflow owns the bridge via existing `runtimeInputDeferredFor(contextId,seq)`; deterministic emit-then-wait test (real blocked-pending point + exact deferred, no sleep-race); no forcing cast; SDD #350 §7 proof. Toy `permission-flow-pipeline` capability now EXISTS → next dispatchable toy build (after capability re-check). | permissions / codecs | Permission flow and codec authority remain unsettled. | cat-1/2 production architectural (resolved-direction: B). ACP codec currently does durable-deferred authority inside the codec (verified) vs README "codecs may not own durable permission state" — B aligns impl to the documented contract; TFIND-041 authority-family precedent |
...
| TFIND-040 | **FULLY DECIDED (Gurdas 2026-05-18)** — Q1=A attach at SESSION level (context handle = host-plane leak into session plane), Q2=live-tail-until-scope-close, Q3=`subscribe()`-only-first (`wait.*` enrichments = tracked follow-up), Q4=generic-stream-only (typed conveniences = follow-up sugar). SDD `SDD_FIREGRID_SESSION_OBSERVATION_SURFACE.md` (`sidecar/session-observation`). Impl ready to dispatch (queued behind keystone). **Projection-scope clarification (Gurdas Q):** the observation *element* `RuntimeAgentOutputObservation` is a protocol projection (SCHEMA_PROJECTION_CONTRACT-governed; agent-tool↔client symmetry by default) so the SEMANTICS are plane-neutral and would carry — BUT TFIND-040 decided the **client binding only**; an **agent-tool binding** (`toolName`) of `subscribe()` is a SEPARATE, not-yet-made binding-scope decision (TFIND-036-class plane scrutiny). Client impl MUST keep the observation contract binding-neutral so a future symmetric agent-tool binding is not foreclosed. | client-sdk / observations | Client SDK lacks a per-event session observation surface. | cat-1 (rubric anchor — real client consumer pattern: `session.subscribe()` per-event; correctly SDD-first) |
...
| TFIND-050 | open — **ESCALATED to ARCHITECTURAL framing** (2026-05-18): the scoped `unknown→never` fix was **FALSIFIED** by `155` — `never` *relocates* the breakage (does not cleanly fix both consumer paths). My earlier "design-conformant 2-line fix within coordinator authority / Option (i)" is **WITHDRAWN**. `155` authorized to author a **§0 framing amendment** to `SDD_DURABLE_TABLE_REACT_LIVE_QUERY.md` (provider-seam layer-erasure design decision) → joins the batched signoff bundle (with TFIND-048 #344) for **Gurdas**. #326 rebase/flip **blocked on this decision**. | effect-durable-operators / provider seam | The `DurableTableProvider` seam cannot erase the layer `ROut` (contravariant) such that BOTH consumer paths typecheck: the flamecast JSX-inference path AND the explicit-props/by-name path. `unknown` breaks by-name; `never` relocates the breakage. Surfaced via #348 (TFIND-044 Option B) during the #326 rebase. | cat-2 **architectural** (NOT a mechanical correction — proven; a real layer-variance design decision the TFIND-044 Option B framing did not resolve). `155` HALTED + falsified the scoped fix instead of papering — correct discipline. Process note: full-CI-green missed it (no by-name test); latent until #326 rebase. Cross-ref TFIND-044 |
