...
const gatewayContextId = "session:tiny-firegrid:mcp-tasks-gateway-parent"
...
const dispatchGeneratedSessionPrompt = (
  ...
  Effect.gen(function*() {
    ...
    Effect.withSpan("tiny_firegrid.mcp_tasks.generated_session_prompt.append", {
      attributes: {
...
const registerToolkitLayer = Layer.scopedDiscard(
  Effect.gen(function*() {
    ...
    Effect.withSpan("tiny_firegrid.mcp_tasks.register_toolkit", {
      attributes: {
...
const McpTasksServerLive = (
  env: TinyFiregridHostEnv,
) =>
  Layer.unwrapEffect(
    Effect.gen(function*() {
      yield* createWireStreams({
      ...
      return registerToolkitLayer.pipe(
        ...
        Layer.provide(makeMcpTasksProtocolLayer(
          {
            baseUrl: env.durableStreamsBaseUrl,
            namespace: env.namespace,
            streamId: env.simulationId,
          },
          contextId => sessionAgentOutput.forContext(contextId).binding.stream,
          request =>
...
export const mcpTasksGatewayHost = (
  env: TinyFiregridHostEnv,
