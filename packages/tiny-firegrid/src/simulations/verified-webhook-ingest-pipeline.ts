...
export const verifiedWebhookIngestSimulation = {
  ...
  summarize: result => ({
    publicSurfaceExpressible: result.publicSurfaceExpressible,
    hmacVerifyOnPublicSurface: result.hmacVerifyOnPublicSurface,
    productHmacVerified: result.productHmacVerified,
    deterministicSourceKey: result.deterministicSourceKey,
    idempotentInsertOrGet: result.idempotentInsertOrGet,
    conflictRejection: result.conflictRejection,
    factObservableViaPublicWait: result.factObservableViaPublicWait,
    evidence: result.evidence,
  }),
  localize: result =>
    result.publicSurfaceExpressible
      ? [
        "Capability PROVEN: external-event -> durable verified fact is",
        "expressible end-to-end through the public Firegrid surface.",
      ]
      : [
        "FINDING (factory-vision §8): external-event -> durable verified fact",
        "is NOT cleanly expressible through the public Firegrid surface.",
        `hmacVerifyOnPublicSurface=${result.hmacVerifyOnPublicSurface} `,
        `deterministicSourceKey=${result.deterministicSourceKey} `,
        `idempotentInsertOrGet=${result.idempotentInsertOrGet} `,
        `conflictRejection=${result.conflictRejection} `,
        `factObservableViaPublicWait=${result.factObservableViaPublicWait}.`,
        "Expressible publicly: deterministic [source,externalEventKey] handle",
        "and idempotent insert-or-get via sessions.createOrLoad.",
        "NOT expressible without reaching past the public surface: HMAC verify",
        "(product/runtime-owned, absent from public surface), payload-hash",
        "CONFLICT REJECTION (createOrLoad keys only on [source,id], no payload",
        "binding — differing payload is silently aliased), and a verified-fact",
        "WAIT (only agent-output/permission waits exist publicly).",
        "Ownership unchanged: this is a substrate-gap finding, not a redesign.",
      ],
