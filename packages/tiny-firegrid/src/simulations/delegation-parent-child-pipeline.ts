...
export const delegationParentChildSimulation = {
  ...
  summarize: result => ({
    ...
    sawSessionNew: result.sawSessionNew,
    sawSessionPrompt: result.sawSessionPrompt,
    sawForbiddenSpawnTool: result.sawForbiddenSpawnTool,
    childCreatedWithParentCorrelation: result.childCreatedWithParentCorrelation,
    observedCreatedBy: result.observedCreatedBy,
    childResultObserved: result.childResultObserved,
    childResumeObserved: result.childResumeObserved,
    childTextExcerpt: result.childText.slice(0, 400),
  }),
  localize: result => {
    if (result.sawForbiddenSpawnTool) {
      return [
        "A spawn/spawn_all ToolUse was observed; §7.4 delegation must use session_new + session_prompt only (finding tf-mn2).",
      ]
    }
    if (!(result.sawSessionNew && result.sawSessionPrompt)) {
      return [
        "The parent's session_new + session_prompt delegation moves were not both observed through the public client.",
        "Inspect the stdio-jsonl codec and runtime tool-use routing spans for the parent context.",
      ]
    }
    if (!result.childCreatedWithParentCorrelation) {
      return [
        `Child context was not observed durably created with parent correlation (expected createdBy 'agent-tool:<parent>', observed '${result.observedCreatedBy}').`,
        "Inspect firegrid.host.agent_tool.session_new and the control-plane context insert path.",
      ]
    }
    if (!result.childResumeObserved) {
      return [
        "Child was created with parent correlation but the session_prompt resume result was not observed from the outside.",
        "Inspect the child context per-context output journal and the session_prompt ingress append path.",
      ]
    }
    return [
      "Full §7.4 delegation move observed end to end through the public surface: durable parent correlation, outside-observable child, inside-resumable child.",
    ]
  },
