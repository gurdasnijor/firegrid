...
export const actionSurvivesRestartSimulation = {
  ...
  driver,
  summarize: result => ({
    sawExecuteToolUse: result.sawExecuteToolUse,
    sawActionResult: result.sawActionResult,
    evidenceRecordedPreRestart: result.evidenceRecordedPreRestart,
    runtimeRestarted: result.runtimeRestarted,
    evidenceSurvivedRestart: result.evidenceSurvivedRestart,
    postRestartActionSucceeded: result.postRestartActionSucceeded,
    contextRecoveredPostRestart: result.contextRecoveredPostRestart,
    observedResultExcerpt: result.observedResult.slice(0, 300),
  }),
  localize: result => {
    if (!result.sawActionResult) {
      return [
        "The provider-edge #388 execute action result was not observed through the public client in gen-1.",
        "Inspect firegrid.host.agent_tool.execute + SandboxProvider spans and the stdio-jsonl tool-use routing.",
      ]
    }
    if (!result.runtimeRestarted) {
      return [
        "The action result was recorded but the gen-1 -> gen-2 restart sentinel was never observed.",
        "Inspect the makeHost barrier + gen-1 teardown path.",
      ]
    }
    if (!(result.evidenceSurvivedRestart && result.postRestartActionSucceeded)) {
      return [
        "The host restarted but the durably-recorded prior action result was NOT visible to the next decision post-restart — §6 provider-edge durability gap.",
        "Inspect the app-owned durable-streams evidence collection across the host-generation boundary.",
      ]
    }
    return [
      "Full §6 provider-edge durability proven: real execute action -> durable evidence -> gen-1->gen-2 restart -> next decision still sees the prior action result through the public/durable surface.",
    ]
  },
