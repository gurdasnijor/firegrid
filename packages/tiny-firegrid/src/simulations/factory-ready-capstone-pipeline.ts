...
export const factoryReadyCapstoneSimulation = {
  ...
  summarize: result => ({
    ...
    sawPlanText: result.sawPlanText,
    sawPlanApprovalWait: result.sawPlanApprovalWait,
    sawSessionNew: result.sawSessionNew,
    sawSessionPrompt: result.sawSessionPrompt,
    sawChildObservationWait: result.sawChildObservationWait,
    sawTurnComplete: result.sawTurnComplete,
    sawTerminal: result.sawTerminal,
    childSessionId: result.childSessionId,
    childContextObserved: result.childContextObserved,
    childOutputObserved: result.childOutputObserved,
    childObservationErrors: result.childObservationErrors,
    observedToolNames: result.observedToolNames,
    observedToolInputs: result.observedToolInputs,
    proof: result.proof,
  ...
  localize: result =>
    result.proof.factoryReady
      ? [
        "factory-vision §5 minimal slice proven through public Firegrid surfaces: app-owned trigger fact, durable parent context, planner-issued wait_for approval gate, session_new/session_prompt delegation, child output observation, in-sequence child-observation fact, and terminal projection.",
      ]
      : [
        "factory-vision §5 minimal slice was not fully proven.",
        ...result.findings,
        "Inspect ToolUse spans, CallerFact wait-router spans, session_new/session_prompt host spans, and factoryReadyCapstone fact rows.",
      ],
