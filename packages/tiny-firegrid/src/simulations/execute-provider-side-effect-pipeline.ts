...
export const executeProviderSideEffectSimulation = {
  ...
  summarize: result => ({
    ...
    sawProviderStdout: result.sawProviderStdout,
    resultTextExcerpt: result.resultText.slice(0, 600),
  }),
  localize: result => {
    if (!result.sawExecuteToolUse) {
      return [
        "The deterministic agent's `execute` ToolUse was never observed through the public client.",
        "Inspect the stdio-jsonl codec and runtime tool-use routing spans.",
      ]
    }
    if (!result.sawProviderStdout) {
      return [
        "The `execute` ToolUse was observed but the provider stdout sentinel did not flow back as agent output.",
        "Inspect firegrid.host.agent_tool.execute and the SandboxProvider getOrCreate/execute spans: the provider side-effect did not run or its ExecutionResult was not returned through the ToolResult roundtrip.",
      ]
    }
    return [
      "Full execute provider side-effect path observed end to end through the public surface.",
      "Inspect the DuckDB span tables for the host agent-tool execute, SandboxProvider, and ToolResult roundtrip path.",
    ]
  },
