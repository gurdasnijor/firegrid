...
export const sessionLifecycleUnwindSimulation = {
  ...
  summarize: result => ({
    ...
    terminalObserved: result.terminalObserved,
  }),
  localize: result =>
    result.sawReady && !result.terminalObserved
      ? [
        "Agent reached Ready and called session_cancel, but no terminal state was observed through the public client within the deadline.",
        "FINDING CANDIDATE: durable session-lifecycle terminate did not propagate to the public surface — inspect control_request.lifecycle.reconcile and engine.close spans.",
      ]
      : !result.sawReady
      ? [
        "Agent never reached Ready; inspect stdio-jsonl codec and host launch spans.",
      ]
      : [
        "Clean unwind observed. Inspect the DuckDB span tables for control_request.lifecycle.reconcile, engine.close, and the stdio-jsonl Terminated path.",
      ],
