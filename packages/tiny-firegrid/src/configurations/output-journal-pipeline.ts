...
interface OutputJournalPipelineOptions {
...
export const tinyOutputJournalPipeline = (
  options: OutputJournalPipelineOptions,
): Layer.Layer<FiregridHost, DurableTableError> => {
  const namespace = options.namespace ?? `tiny-output-journal-${crypto.randomUUID()}`
