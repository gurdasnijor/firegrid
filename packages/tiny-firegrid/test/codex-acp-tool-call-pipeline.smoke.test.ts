...
import {
  ...
  type RecordedSpan,
} from "../src/simulations/trace-recorder.ts"
import {
  writeTinyFiregridTraceRun,
} from "../src/simulations/trace-artifacts.ts"
...
const writeTraceArtifact = async (
  ...
  const agentReportedExpectedResult = input.observed.resultText.includes("FIREGRID_TOOL_RESULT")
  const summary = {
  ...
  const localization = observedNoToolUse
    ? [
      ...
      agentReportedToolUnavailable
        ...
        : "The real Codex ACP agent nevertheless reaches Ready without Firegrid observing a ToolUse event.",
  ...
  await writeTinyFiregridTraceRun({
    configuration: "codex-acp-tool-call-pipeline",
    source: "packages/tiny-firegrid/test/codex-acp-tool-call-pipeline.smoke.test.ts",
    markdownTitle: "G-MCP-2 MCP Tool Exposure Trace",
    summary,
    localization,
    fibers: input.fibers,
    spans: input.spans,
    legacyArtifacts: {
      markdownPath: traceArtifactPath,
      jsonPath: traceJsonArtifactPath,
    },
  })
}
