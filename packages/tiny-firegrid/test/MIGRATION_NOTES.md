...
- The durable-streams-backed test now validates startup reconciliation without host internals: materialize a context through `sessions.createOrLoad` plus the public host, append a prompt while no host is active, then call `session.start()` and launch a new host to prove `RuntimeContextEngineRegistry.reconcile` drains the durable input intent into the per-context engine.
