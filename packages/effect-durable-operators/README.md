...
- **`DurableTable`** — a queryable, last-write-wins table derived from a
  durable State-Protocol change-event stream. Declare it as an Effect service
  tag with row schemas, then bind it to a stream with `.layer(...)`.
...
`DurableTable(...)` returns an Effect service-tag class. Collection durable
types are convention-derived as `<tableNamespace>.<collectionKey>`, so
renaming either value is a replay-breaking migration in v0.
...
const program = Effect.gen(function* () {
  ...
  const updates = table.executions.subscribe((coll, emit) => {
  ...
  return { execution, rows, updates }
})

const Live = WorkflowTable.layer({
  streamOptions: {
    url: "https://durable-streams.example/workflow-state",
    contentType: "application/json",
  },
})
```

Each collection exposes `insert`, `upsert`, `delete`, `get`, `query`, and
`subscribe`. Standard writes are generated by convention through upstream
`createStreamDB({ actions })`; callers do not pass an `actions:` config for
the common insert/upsert/delete path. Generated writes use the
`createStateSchema` collection helpers to construct State Protocol events, so
primary-key extraction and event headers stay aligned with `@durable-streams/state`.

`query` receives the underlying TanStack collection. Use TanStack query
operators from `@tanstack/db`, or the same re-exported operators from
`@durable-streams/state`, when building filters.
...
current state) with a collection `subscribe` stream for future updates, and
let `@effect/workflow` complete its own `DurableDeferred` from the matched
value.
...
const waitForPermit = (
  ...
  Effect.gen(function* () {
    ...
    const existing = yield* table.permits.get(requestId)
    ...
    //    job, not ours.
    const changes = table.permits.subscribe<Permit>((coll, emit) => {
      const sub = coll.subscribeChanges(
        (chs) => {
          for (const c of chs) {
...
The package gives you the *primitives* (`get`, `subscribe`, last-write-wins
...
class ChildTerminalTable extends DurableTable("spawn.child", {
  terminals: Schema.Struct({
    childId: Schema.String.pipe(DurableTable.primaryKey),
...
const allDone = (
  ...
  Effect.gen(function* () {
    const rows = yield* table.terminals.query((c) => c.toArray)
...
| Module | Surface |
| --- | --- |
| `DurableTable` | `DurableTable(namespace, schemas)`, `DurableTable.primaryKey`, service-tag `.layer(...)`, collection facades with `insert` / `upsert` / `delete` / `get` / `query` / `subscribe` |
