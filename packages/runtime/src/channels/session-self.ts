...
// tf-bffo / tf-77ab: the durable session-self channel implementations (the
// control-plane run lifecycle stream + the workflow-engine checkpoint snapshot/stream
// reads) live in the runtime. host-sdk only COMPOSES the Live Layer by resolving the
// control table + the checkpoint source from context and delegating here.

// Structural mirror of the host-side checkpoint source/handle (all fields are
// runtime/protocol types, so the runtime owns this shape without importing host-sdk).
export interface SessionSelfCheckpointHandle {
  readonly context: RuntimeContext
  readonly executionId: string
  readonly table: WorkflowEngineTable["Type"]
}

export interface SessionSelfCheckpointSource {
  readonly get: (
    contextId: string,
  ) => Effect.Effect<Option.Option<SessionSelfCheckpointHandle>>
...
const checkpointsForEngine = (
  handle: SessionSelfCheckpointHandle,
...
const checkpointStream = (
  checkpoints: SessionSelfCheckpointSource,
): Stream.Stream<SessionSelfCheckpointEvent, unknown, never> =>
  Stream.unwrap(
    Effect.gen(function* () {
...
export const makeSessionSelfChannels = (
  options: {
    ...
    readonly checkpoints: SessionSelfCheckpointSource
  },
): readonly [
