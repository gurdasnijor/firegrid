/**
 * Runtime-owned Firegrid CLI entrypoint.
 *
 * This module owns the actual `firegrid run | start | acp` behavior.
 * The thin `@firegrid/cli` launcher subprocesses into it; the runtime
 * package is the only one that may import runtime substrate (per
 * `2026-05-22-runtime-physical-target-tree.md` "Composition Boundary"
 * and the Shape C cutover roadmap "Runtime/bin may import runtime
 * internals" rule).
 *
 * Usage (typically invoked by the CLI launcher):
 *
 *   tsx packages/runtime/src/bin/run.ts run -- node -e 'console.log(...)'
 *   tsx packages/runtime/src/bin/run.ts start
 *   tsx packages/runtime/src/bin/run.ts acp --agent codex-acp ...
 *
 * Behavior is preserved verbatim from the previous CLI source
 * `packages/cli/src/bin/run.ts`; only the import edges are re-rooted at
 * runtime canonical homes.
 *
 * Implements:
 *  - firegrid-workflow-driven-runtime.PHASE_2_SYNC_RUN.1..8
 *  - firegrid-workflow-driven-runtime.VALIDATION.2
 *  - firegrid-local-mcp-run.LOCAL_COMMAND.1..5
 *  - firegrid-local-mcp-run.EMBEDDED_DURABLE_STREAMS.1..3
 *  - firegrid-local-mcp-run.MCP_ROUTE.1..2
 *  - firegrid-local-mcp-run.CLI_HELP.1..4
 *  - firegrid-local-mcp-run.EFFECT_COMPOSITION.1..3
 *  - firegrid-local-mcp-run.AUTHORITY_BOUNDARY.1..2
 *  - firegrid-zed-acp-stdio-external-agent.ACP_STDIO_EDGE.5
 *  - firegrid-zed-acp-stdio-external-agent.CLI_HELPER.3..4
...
// Runtime canonical homes — runtime/bin is the one place we may reach
// directly into runtime substrate.
import {
  appendRuntimeIngress,
  startRuntime,
} from "@firegrid/runtime/composition/host-public"
import {
...
} from "@firegrid/runtime/composition/sync-run"
import {
...
// CLI session identity. Each `firegrid run`/`start`/`acp` invocation is a
// fresh caller-owned session; the externalKey id is a generated uuid and
// the source distinguishes the entrypoint. TFIND-048: the CLI no longer
// pre-derives the contextId to bake an MCP URL — the host owns the
// runtime-context MCP URL and injects it post-materialization at start.
// The CLI only expresses the URL-less marker.
...
// TFIND-048: request the host-owned runtime-context MCP server via the
// URL-less marker. Both `firegrid run` and `firegrid start` attach it by
// default; the host resolves and injects the concrete contextId-scoped
// URL at start.
