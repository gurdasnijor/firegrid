...
// `./decode-ingress-row.ts`. The pure decoder returns `Either`, which in
...
//
// Deletion blocker: this shim drops when the remaining
// `.pipe(Effect.mapError(...))` callers (tiny-firegrid host) migrate to
// `Either.mapLeft` / direct Effect-subtype use over the sibling
// `decode-ingress-row.ts` symbol.
...
import { agentInputEventFromRuntimeIngressRow as decodeAgentInputEventEither } from "./decode-ingress-row.ts"
import type { RuntimeIngressAgentInputTransformError } from "./decode-ingress-row.ts"
