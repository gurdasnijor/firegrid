...
const upsertActiveWait = (
  ...
  Effect.gen(function*() {
    const existing = yield* Effect.mapError(
      findWaitByKey(table, { executionId: row.executionId, name: waitName }),
      (cause) =>
        waitForError({
          op: "wait-for/upsert",
          waitName,
    ...
    yield* Effect.mapError(table.waits.upsert(row), (cause) =>
      waitForError({
        op: "wait-for/upsert",
        waitName,
...
const writeTimeoutCompletion = (
  ...
  Effect.gen(function*() {
    ...
    yield* Effect.mapError(
      table.waits.upsert({ ...current, status: "timed_out" }),
      (cause) =>
        waitForError({
          op: "wait-for/timeout",
          waitName,
