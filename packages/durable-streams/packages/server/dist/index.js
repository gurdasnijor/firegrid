...
var ConsumerStore = class {
	...
	*/
	registerConsumer(consumerId, streams, getTailOffset, opts) {
		...
		for (const path$1 of streams) streamMap.set(path$1, INITIAL_CONSUMER_OFFSET);
		const consumer = {
		...
		for (const path$1 of streams) this.addStreamIndex(path$1, consumerId);
		return {
			consumer,
			created: true
		};
	...
	removeConsumer(consumerId) {
		...
		for (const path$1 of consumer.streams.keys()) this.removeStreamIndex(path$1, consumerId);
	...
	*/
	updateOffsets(consumer, offsets, getTailOffset) {
		for (const { path: path$1, offset } of offsets) {
			const current = consumer.streams.get(path$1);
			...
			const tail = getTailOffset(path$1);
			if (compareOffsets$1(offset, tail) > 0) return {
				path: path$1,
				code: `INVALID_OFFSET`
			};
		}
		for (const { path: path$1, offset } of offsets) if (consumer.streams.has(path$1)) consumer.streams.set(path$1, offset);
		consumer.last_ack_at = Date.now();
		return null;
	}
	/**
	...
	*/
	addStreams(consumer, paths, getTailOffset) {
		for (const path$1 of paths) if (!consumer.streams.has(path$1)) {
			consumer.streams.set(path$1, getTailOffset(path$1));
			this.addStreamIndex(path$1, consumer.consumer_id);
		}
	}
	/**
	...
	*/
	removeStreams(consumer, paths) {
		for (const path$1 of paths) {
			consumer.streams.delete(path$1);
			this.removeStreamIndex(path$1, consumer.consumer_id);
		}
	...
	*/
	getStreamsData(consumer) {
		return Array.from(consumer.streams, ([path$1, offset]) => ({
			path: path$1,
			offset
		}));
	}
	...
	*/
	hasPendingWork(consumer, getTailOffset) {
		for (const [path$1, ackedOffset] of consumer.streams) {
			const tail = getTailOffset(path$1);
...
function signWebhookPayload(body, secret) {
	...
	if (secret) {
		const signature$1 = createHmac(`sha256`, secret).update(payload).digest(`hex`);
...
var ConsumerRoutes = class {
	...
	*/
	async handleRequest(method, path$1, req, res) {
		if (!path$1.startsWith(`/consumers`)) return false;
		if (path$1 === `/consumers` && method === `POST`) {
			await this.handleRegister(req, res);
			return true;
		}
		const segments = path$1.slice(`/consumers/`.length).split(`/`);
	...
	async handleRegister(req, res) {
		...
		if (!Array.isArray(payload.streams) || payload.streams.length === 0 || payload.streams.some((path$1) => typeof path$1 !== `string` || path$1.length === 0)) {
			res.writeHead(400, { "content-type": `application/json` });
			res.end(JSON.stringify({ error: {
				code: `INVALID_REQUEST`,
...
const tracer = trace.getTracer(`durable-streams.webhook`);
		...
		try {
			...
			if (response.ok) {
				...
				if (resBody?.done) {
					...
					const l1Consumer = this.consumerManager.store.getConsumer(wc.consumer_id);
					if (l1Consumer) {
						const tailOffsets = Array.from(l1Consumer.streams.keys()).map((path$1) => ({
							path: path$1,
							offset: this.getTailOffset(path$1)
						}));
	...
	*/
	async handleCallback(consumerId, token, request) {
		...
		if (request.subscribe) {
			...
			for (const path$1 of request.subscribe) this.store.addStreamIndex(path$1, consumerId);
		}
		if (request.unsubscribe) {
			const shouldRemove = this.consumerManager.store.removeStreams(l1Consumer, request.unsubscribe);
			for (const path$1 of request.unsubscribe) this.store.removeStreamIndex(path$1, consumerId);
			if (shouldRemove) {
				callbackSpan.end();
...
var WebhookRoutes = class {
	...
	*/
	async handleRequest(method, url, path$1, req, res) {
		if (path$1.startsWith(`/callback/`)) {
			await this.handleCallback(path$1, req, res);
			return true;
		}
		...
		if (hasSubscription) {
			...
			switch (method) {
				case `PUT`:
					await this.handleCreateSubscription(path$1, subscriptionId, req, res);
					return true;
				case `GET`:
		...
		if (hasSubscriptions && method === `GET`) {
			this.handleListSubscriptions(path$1, res);
			return true;
		}
		return false;
	}
	...
	async handleCallback(path$1, req, res) {
		const consumerId = path$1.slice(`/callback/`.length);
		const authHeader = req.headers[`authorization`];
		if (!authHeader || !authHeader.startsWith(`Bearer `)) {
			res.writeHead(401, { "content-type": `application/json` });
			res.end(JSON.stringify({
...
var DurableStreamTestServer = class {
	...
	async start() {
		...
		return new Promise((resolve, reject) => {
			...
			this.server.listen(this.options.port, this.options.host, () => {
				...
				this.consumerManager = new ConsumerManager({ getTailOffset: (path$1) => {
	...
	*/
	...
	async handleRequest(req, res) {
		...
		if (this.consumerRoutes && method) {
			const handled = await this.consumerRoutes.handleRequest(method, path$1, req, res);
			if (handled) return;
		}
		if (this.webhookRoutes && method) {
			const handled = await this.webhookRoutes.handleRequest(method, url, path$1, req, res);
			if (handled) return;
		}
		try {
			switch (method) {
	...
	*/
	async handleCreate(path$1, req, res) {
		...
		if (isNew && this.webhookManager) this.webhookManager.onStreamCreated(path$1);
	...
	*/
	async handleAppend(path$1, req, res) {
		...
		if (body.length === 0 && closeStream) {
			...
			const closeResult = await Promise.resolve(this.store.closeStream(path$1));
			if (!closeResult) {
				res.writeHead(404, { "content-type": `text/plain` });
				res.end(`Stream not found`);
				return;
	...
	async notifyStreamAppend(path$1) {
		...
		if (this.webhookManager) try {
			this.webhookManager.onStreamAppend(path$1);
		} catch (err) {
			serverLog.error(`[server] webhook append hook failed:`, err);
		}
		if (this.pullWakeManager) try {
			this.pullWakeManager.onStreamAppend(path$1);
		} catch (err) {
	...
	*/
	async handleDelete(path$1, res) {
		...
		if (this.consumerManager) this.consumerManager.onStreamDeleted(path$1);
		if (this.webhookManager) this.webhookManager.onStreamDeleted(path$1);
		res.writeHead(204);
		res.end();
	}
	/**
