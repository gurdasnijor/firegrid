...
const HOST_SUBSTRATE_NAMES: ReadonlySet<string> = new Set([
  ...
  "fluent-acp-process.stdin_bytes",
  "fluent-acp-process.stdout_bytes",
  // fluent-runtime ACP callbacks: host-side client callback entrypoints before
  // they are persisted as fluent-runtime store events.
  "fluent_runtime.acp.session_update",
  "fluent_runtime.acp.request_permission",
  "fluent_runtime.acp.ext_method",
])
...
// Claims every host-launching sim wants — shared so they don't drift. firegrid
// analogs of flamelab's everyTurnCallsModel / turnsDidNotError.
export const sessionDroveWorkflow: ClaimDef = {
  id: "session.drove_workflow",
  description: "a session workflow body executed on the engine",
  claim: "spans.exists(s, named(s, \"firegrid.workflow_engine.execution.execute\"))",
}
export const adapterStartedAgent: ClaimDef = {
