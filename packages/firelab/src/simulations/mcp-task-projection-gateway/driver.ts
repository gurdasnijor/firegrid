...
const gatewayContextId = "session:firelab:mcp-task-projection-gateway-gateway"
...
export const mcpTaskProjectionGatewayDriver: Effect.Effect<void, unknown, FiregridConfig> =
  Effect.scoped(Effect.gen(function*() {
    ...
    const promptTask = yield* session.promptTask({
      prompt: promptText(
        `packages/firelab/.simulate/mcp-task-projection-permission-probe-${globalThis.crypto.randomUUID()}.txt`,
      ),
      inputId: "firelab-mcp-task-projection-prompt-1",
      taskTtlMs: 120_000,
    })
    ...
    Effect.withSpan("firelab.mcp_task_projection.driver", {
      kind: "client",
    }),
  )
