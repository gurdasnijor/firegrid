...
// externalKey is { source: "firelab", id: "mcp-client-sdk-gateway-gateway" }.
const gatewayContextId = "session:firelab:mcp-client-sdk-gateway-gateway"
...
export const mcpClientSdkGatewayDriver: Effect.Effect<void, unknown, FiregridConfig> =
  Effect.scoped(Effect.gen(function*() {
    ...
    const sessionPromptTask = yield* child.promptTask({
      prompt: promptText(
        `packages/firelab/.simulate/mcp-client-sdk-gateway-${globalThis.crypto.randomUUID()}.txt`,
      ),
      inputId: "firelab-mcp-client-sdk-gateway-prompt-1",
    })
    ...
    Effect.withSpan("firelab.mcp_client_sdk_gateway.driver", {
      kind: "client",
    }),
  )
