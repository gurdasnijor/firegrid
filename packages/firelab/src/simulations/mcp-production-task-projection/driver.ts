...
const gatewayContextId =
  "session:firelab:mcp-production-task-projection-gateway"
...
export const mcpProductionTaskProjectionDriver: Effect.Effect<void, unknown, FiregridConfig> =
  Effect.scoped(Effect.gen(function*() {
    ...
    const permissionProbePath =
      `packages/firelab/.simulate/mcp-production-task-projection-${globalThis.crypto.randomUUID()}.txt`
    const promptText = [
    ...
    const promptTask = yield* session.promptTask({
      prompt: promptText,
      inputId: "firelab-mcp-production-task-projection-prompt-1",
      taskTtlMs: 120_000,
    })
    ...
    Effect.withSpan("firelab.mcp_production_task_projection.driver", {
      kind: "client",
    }),
  )
