...
const gatewayContextId =
  "session:firelab:mcp-client-sdk-observations-gateway"
...
export const mcpClientSdkObservationsDriver: Effect.Effect<void, unknown, FiregridConfig> =
  Effect.scoped(Effect.gen(function*() {
    ...
    const sessionPromptTask = yield* child.promptTask({
      prompt: promptText(
        `packages/firelab/.simulate/mcp-client-sdk-observations-${globalThis.crypto.randomUUID()}.txt`,
      ),
      inputId: "firelab-mcp-client-sdk-observations-prompt-1",
    })
    ...
    Effect.withSpan("firelab.mcp_client_sdk_observations.driver", {
      kind: "client",
    }),
  )
