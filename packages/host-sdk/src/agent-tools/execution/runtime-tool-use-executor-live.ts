...
import { ChannelInventory } from "../../host/channel.ts"
import {
  toolErrorResult,
  toolExecutionFailed,
...
export const RuntimeToolUseExecutorLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const channelInventory = Context.get(captured, ChannelInventory)
    ...
    return RuntimeToolUseExecutor.of({
      execute: (context, event) =>
        toolUseToEffect(context, event).pipe(
          ...
          Effect.provideService(ChannelInventory, channelInventory),
