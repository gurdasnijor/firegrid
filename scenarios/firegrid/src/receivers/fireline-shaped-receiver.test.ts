...
describe("FW2 Fireline-shaped happy-path receiver scenario", () => {
  it("firegrid-runtime-process.SCENARIOS.13, firegrid-runtime-process.SCENARIOS.16, firegrid-runtime-process.RUNTIME_COMPOSITION.1, firegrid-runtime-process.RUNTIME_COMPOSITION.2, firegrid-runtime-process.RUNTIME_COMPOSITION.6, client-event-plane-registration.BOUNDARY.5, run-wait-primitives.RUN_WAIT_API.1, run-wait-primitives.RUN_WAIT_API.2, run-wait-primitives.RUN_WAIT_API.6, run-wait-primitives.BOUNDARY.4, run-wait-primitives.BOUNDARY.5 — app-owned RunWait receiver composes explicit helper inputs, resolves projection-match, and terminalizes ready work", async () => {
