...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      from: {
        ...
        pathNot: [
          ...
          '(^|/)src/index[.](?:ts|tsx|js)$',                                   // package entrypoints
          '(^|/)__tests__/'                                                    // test files (exercised by vitest, not imported)
        ]
      },
      to: {},
    },
    ...
    {
      name: 'substrate-is-foundational',
      severity: 'error',
      comment:
        'packages/substrate is the foundation layer; it must not import from client/host/lab. ' +
        'If you need shared logic going the other way, lift it into substrate.',
      from: { path: '^packages/substrate/' },
      to:   { path: '^packages/(client|host|lab)/' },
    },
    {
      name: 'client-no-host-or-lab',
      severity: 'error',
      comment:
        'packages/client must not depend on host or lab — it sits below them in the layering.',
      from: { path: '^packages/client/' },
      to:   { path: '^packages/(host|lab)/' },
    },
    {
      name: 'peer-deps-used',
      comment:
    ...
    reporterOptions: {
      dot: {
        ...
        theme: {
          ...
          modules: [
            { criteria: { source: '^packages/substrate/' }, attributes: { fillcolor: '#e3f2fd' } },
            { criteria: { source: '^packages/host/'      }, attributes: { fillcolor: '#fff3e0' } },
            { criteria: { source: '^packages/client/'    }, attributes: { fillcolor: '#f3e5f5' } },
            { criteria: { source: '^packages/lab/'       }, attributes: { fillcolor: '#e8f5e9' } },
          ],
        },
      },
      ddot: {
        // one node per second-level folder inside each package's src/
...
// generated: dependency-cruiser@17.4.0 on 2026-05-05T05:04:15.841Z
