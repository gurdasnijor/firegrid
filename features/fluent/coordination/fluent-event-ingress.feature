...
Feature: Fluent event ingress
  ...
  after delivery or claim. Provider webhooks are admitted as product facts; they
  are not the Durable Streams webhook-wake protocol.
  ...
  Scenario: Accepted external delivery becomes a durable event
    ...
    And the provider delivery is not treated as a Durable Streams subscription webhook wake
  ...
  Scenario: Matching provider event records a wait match
    ...
    And Durable Streams delivers or grants subscribed work to a post-claim actor
    Then the post-claim actor materializes the candidate event and waiting session facts
    And Firegrid records a Layer 2 wait_matched fact for the waiting session
    And Firegrid acks or dones only after wait_matched is durable
  ...
  Scenario: Non-matching provider event does not match unrelated waits
    ...
    Then Firegrid evaluates product wait semantics above the substrate
    And the wait remains pending
    And Durable Streams is not asked to evaluate the Firegrid CEL predicate
