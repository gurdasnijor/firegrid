...
| `firegrid.prompt` / `HostPromptChannel` | A | Top-level prompt decodes, waits via `awaitSessionDependentContext(decoded.contextId)` **bounded by `Effect.timeoutOption(config.contextReflectionTimeoutMs)` (default 30s)**, then appends through `HostPromptChannel` (`packages/client-sdk/src/firegrid.ts`, `prompt` body). The wait lowers to `HostContextsChannel.binding.stream` (`packages/client-sdk/src/firegrid.ts:744`, `:751`, `:758`). The append binding still enforces context existence before `inputIntents.insertOrGet` (`packages/protocol/src/launch/host-control-request.ts:62`, `:67`, `:72`); the bounded wait means an unknown/typo context id falls through on timeout to that check and surfaces `ContextNotFound` (mapped to `AppendError`) **instead of hanging forever**. | New closure test forks `firegrid.prompt` before context materialization, materializes later, and observes a stored prompt intent (`packages/client-sdk/test/firegrid.sessions.test.ts:478`, `:494`, `:500`, `:509`, `:515`). Bounded-error test drives an unknown context id with a short reflection window and asserts a bounded `AppendError`/`ContextNotFound`, not a hang (`packages/client-sdk/test/firegrid.sessions.test.ts`, "unknown context id errors (bounded) instead of hanging"). Receipt test still proves returned row equals stored row (`packages/client-sdk/test/firegrid.sessions.test.ts:437`, `:447`, `:475`). | This was the live gap. It is now closed with the same dependent-operation barrier used by session-scoped prompt, with the barrier explicitly **bounded** so invalid ids reach the existing context-existence error path rather than waiting indefinitely. |
...
1. **Implemented in this PR: top-level prompt reflection barrier (bounded).**
   `firegrid.prompt` now waits for reflected context state before
   `HostPromptChannel` append. The wait is bounded by
   `config.contextReflectionTimeoutMs` (default 30s): a real in-flight
   context materializes within the window, but an unknown/typo context id
   never materializes, so on timeout the prompt falls through to the
   append's context-existence check and surfaces `ContextNotFound`
   (mapped to `AppendError`) rather than hanging indefinitely. The focused
   tests cover `launch -> firegrid.prompt` without explicit `whenReady`
   AND the bounded-error path for an unknown context id.
