...
Local development runs Durable Streams outside Firegrid and passes a
concrete stream URL to the runtime or lab. The Firegrid runtime does not
own `durable-streams-server dev`, does not create streams as a side effect
of process startup, and does not spawn child dev processes.
...
2. User-visible CLI output should use `@effect/platform/Terminal`.
3. Runtime lifetime should be `Effect.scoped`.
4. The platform dependencies should be provided by `@effect/platform-node`
...
The binary should not grow its own process manager. Signal handling,
resource finalization, and terminal IO should come from Effect Platform
unless a concrete gap is identified.
...
4. Firegrid-owned Durable Streams dev-server launchers.
5. Turbo as the semantic process orchestration layer.

Runtime process configuration belongs at the binary process edge:

1. Attached mode uses `DURABLE_STREAMS_URL` or an explicit CLI stream URL.
2. Missing stream URL is a configuration error.
3. Runtime process identity is generated by default and can later be reified in
   durable runtime launch/liveness records. It is not normal user-facing
   environment config in v1.
4. Exported library APIs should take explicit values. `bootPlanFromConfig` /
   `attachedFromConfig`-style public APIs should be removed from the Firegrid
   runtime surface.
...
3. Remove fixed-port coupling from the lab default.
4. Keep Turbo for build caching only, not as the architectural runtime/lab
