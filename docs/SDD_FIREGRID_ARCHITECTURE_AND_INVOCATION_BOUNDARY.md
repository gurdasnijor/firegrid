...
5. Inject environment variables:
   ...
   ```txt
   DURABLE_STREAMS_URL=<resolved-url>
   VITE_DURABLE_STREAMS_URL=<resolved-url>
   ```
...
2. Embedded development mode is selected with `firegrid dev -- <command...>`.
3. Embedded development topology may use explicit CLI flags when needed, but it
   should not require broad environment variables such as Durable Streams host,
   port, stream name, or runtime process id.
4. Runtime process identity is generated by default and can later be reified in
   durable runtime launch/liveness records. It is not normal user-facing
   environment config in v1.
5. Exported library APIs should take explicit values. `bootPlanFromConfig` /
...
3. Add a runtime process runner that injects `DURABLE_STREAMS_URL` into child
   processes.
