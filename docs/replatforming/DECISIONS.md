...
| D-17 | LT-02 can resume on existing public surfaces: the app's Node runtime entrypoint may import both `@firegrid/runtime` and `@firegrid/client`; browser code may read timeline state through `@firegrid/client.events` or app-owned EventPlane projection services from the approved `@firegrid/substrate/event-plane` subpath. No new Firegrid API is required before the first chassis proof. | `firegrid-platform-invariants.LOCALITY.1`, `firegrid-platform-invariants.LOCALITY.5`, `firegrid-agent-runtime-substrate.MULTI_WAIT_RESUME.2`, `firegrid-agent-runtime-substrate.TOPOLOGY_PROFILE.1`, `firegrid-agent-runtime-substrate.TOPOLOGY_PROFILE.2`, `firegrid-client-projection-api.BROWSER_SAFE_FACADE.1` |
