...
### Scenario 2C: Fireline-Shaped Rejection

Purpose: prove the Fireline-shaped negative path without adding product
vocabulary to Firegrid durable row families.

Input:

1. A schema-valid operation-started row for an app-owned
   `FirelineShapedRejection` operation.
2. A schema-valid app-owned `FirelineDecisionEvents` EventStream row with
   `status: "rejected"`.
3. An app-owned runtime Layer with explicit projection-match subscriber and
   handler Layers.
...
1. Runtime attaches through `run({ connection, runtime })`.
2. The handler uses `RunWait.for(trigger, { resultSchema })` rather than
   `Choreography` or raw substrate kernel APIs.
3. Projection-match subscriber resolves the durable completion with app-level
   rejection data from the EventStream row.
4. Ready-work resumes the handler.
5. The handler maps the rejection value to its typed operation error schema.
6. Inspection shows the run failed with `_tag: "FirelineRequestRejected"` and
   `readyWork` count `0`.
...
The inspection report should show `run-fireline-rejection-cli-1` as `failed`,
one `projection_match` completion as `resolved`, the app-owned
`FirelineDecisionEvents` EventStream row, and `readyWork` count `0`.
