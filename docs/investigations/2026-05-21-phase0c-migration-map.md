...
- **F1 — Workflow-owned runtime input table is absent in production.** `RuntimeContextWorkflowNative` has no `inputs`/`events` DurableTable of its own; input arrives via the deferred-name mailbox (S4/S5/S6). Phase 0A proved the shape in `firelab` (`SessionTable.inputs` + `sessions.nextInputSequence`); production needs the equivalent owned table before S2/S4/S6 can cut over. Naming it (e.g. a runtime-context-owned session table with `inputs` + cursor columns) is the Phase 0C design step, not something to bridge around with another deferred family.
