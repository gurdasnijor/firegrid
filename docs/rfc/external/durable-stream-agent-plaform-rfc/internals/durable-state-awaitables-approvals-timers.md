...
Composite waits such as races **MUST** distinguish the composite result from each child awaitable. A race may resolve the composite to the first child completion, but it **MUST NOT** consume, delete, or terminalize the losing child awaitables unless it appends explicit cancellation records for those child keys. Later waiters for a losing child key must still be able to observe that child's eventual completion through the normal snapshot/subscription path. [executable contract: `crates/fireline-substrate/tests/promises_awakeable_race.rs:16`; `crates/fireline-substrate/tests/promises_awakeable_race.rs:84`]
...
terminal-conflict policy. [executable contract:
`crates/fireline-substrate/tests/promises_ds_dp_composite.rs:29`;
`crates/fireline-substrate/tests/promises_ds_dp_composite.rs:145`;
`crates/fireline-substrate/tests/promises_ds_dp_composite.rs:316`;
`crates/fireline-substrate/tests/promises_ds_dp_composite.rs:402`]
...
Tool-call-scoped approval **MUST** gate real tool calls only. A prompt that does not reach a concrete tool call **MUST NOT** be blocked by tool-call approval middleware merely because the session or prompt exists. [executable contract: `tests/observability_agent_plane.rs:528`]
...
result to the caller or projection. [executable contract:
`crates/fireline-substrate/tests/promises_wake_timer_cancel.rs:16`;
`crates/fireline-substrate/tests/promises_wake_timer_cancel.rs:67`]
...
on restart wall-clock time. [executable contract:
`crates/fireline-substrate/tests/promises_awakeable_timeout.rs:132`;
`crates/fireline-substrate/tests/promises_awakeable_timeout.rs:256`]
