...
| --- | --- | --- | --- |
| App-owned operation handlers | Ready | `Firegrid.handler(...)` plus `run({ connection, runtime })` in `scenarios/firegrid/src/receivers/echo-receiver.ts`; runtime API surface in `packages/runtime/src/runtime-api.ts` | None known |
...
- `scenarios/firegrid/src/receivers/firepixel-prompt-chunk-receiver.ts`
  ...
  `Firegrid.handler(...)`, and `run({ connection, runtime })`.
