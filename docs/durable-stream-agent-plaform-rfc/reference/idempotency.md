...
Projection layers **SHOULD** fold duplicate launch/session requests by the logical idempotency key, not by incidental attempt id. Multiple attempts for the same logical idempotency key **MUST** materialize as one logical operation row or a documented conflict, so subscribers do not treat retries as independent launches. [executable contract: `packages/state/test/state-protocol.test.ts:5`; `packages/substrate/test/launch-fold-protection.test.ts:71`]
