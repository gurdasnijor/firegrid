...
- product work projection row, only if the app UI needs state beyond existing
  session/runtime observations;
- planner prompt;
...
with a prompt generated from the actual installed tool catalog and product
policy. This is the only place the product chooses the planner agent:
...
does not create a bespoke trigger coordinator when accepted work facts,
runtime observations, and app-owned projections are all exposed through the
same agent-facing tool surface:

```text
You are the factory planner.

You have these installed Firegrid tools:
<rendered from protocol/tool schemas and host-installed app bindings>

Use wait_for when you need to suspend until a runtime observation or
app-owned fact/projection changes. Use read tools when you need current
durable state. Use execute tools only for installed provider capabilities
whose schemas are shown in the tool catalog.
...
create a product work projection, launch an implementer, wait for CI, or call
GitHub. Those are planner decisions made through the agent tool surface after
`wait_for` observes the accepted fact.
...
the accepted fact key and, if the app needs one, an app-owned work projection
capability. They do not converge through a bespoke server-side trigger
workflow or a second Firegrid run resource.

Per-work context can be read by the planner itself after `wait_for` observes
an accepted fact. If the app exposes a product work projection, that projection
is app state linked to Firegrid session/runtime observations; it is not a
replacement for `RuntimeContext` or `RuntimeRun`:
...
<rendered tool catalog, including schema-backed input/output descriptions>
...
- wait_for: wait on runtime-owned observations such as agent output,
  permission requests, child session output, runtime run state, or app-owned
  fact/projection targets installed by the host
...
```ts
export const readFactoryWork = (workKey: FactoryWorkKey) =>
  Effect.gen(function* () {
    const work = yield* getFactoryWorkProjection(workKey)
    const sessions = yield* FiregridSessions
    const planner = yield* sessions.attach({ sessionId: work.plannerSessionId })
    const snapshot = yield* planner.snapshot()
    const timeline = yield* buildFactoryTimeline(workKey, snapshot)
    return { work, planner: snapshot, timeline }
  })
```
...
| Record or read app-specific work state for an accepted fact? | Planner agent through app-installed read/execute bindings, only if the app needs a UI/product projection beyond Firegrid session/runtime resources. |
...
`DarkFactoryRunSchema` is also more generic than the name suggests, and the
name is misleading in a Firegrid SDK discussion:
...
This is an app-owned product work projection:

- deterministic work identity from a provider/entity key;
...
This is not the same as Firegrid `RuntimeRun`. In
`packages/protocol/src/launch/schema.ts`, `RuntimeRunEvent` is one
provider execution attempt inside a `RuntimeContext`, with statuses
`started`, `exited`, and `failed`. A factory work item, Flamecast toy
scenario, or future product item is caller-owned state that may span multiple
agent sessions, runtime attempts, provider callbacks, permissions, and side
effects. If the product needs that state for UI/idempotency, model it as an
app-owned projection linked to session-plane handles. Do not promote it into a
second Firegrid "run" resource.
...
| `DarkFactoryRunSchema` | Product work projection: one app-owned record for one external work item, linked to one or more Firegrid sessions/runtime attempts when needed. | Restate workflow ID/read model; Temporal workflow query state; Inngest function run keyed by event data. |
...
- **product work projection**: current UI/idempotency state for one external
  work item, if existing session/runtime resources are not enough;
...
ID maps most closely to a product work identity, and durable promises/awakeables
map to factory gates: one handler waits, another handler resolves the external
event. The Firegrid version should keep the same durable resume semantics, but
the product work projection remains app-owned and the agent session/runtime
resources remain separate Firegrid concepts.

Temporal's equivalent vocabulary is workflow execution, signals, queries, and
activities. The product work projection resembles query/read-model state;
provider webhooks and human decisions resemble signals; provider side effects
resemble activities; UI status resembles a query/read model. Firegrid should
learn the separation without importing Temporal's full deterministic workflow
model into the product app. The planner is allowed to choreograph using durable
evidence, not only follow a TypeScript phase graph.
...
### Product Work Projection

```ts
export const FactoryWorkProjectionSchema = Schema.Struct({
  workKey: FactoryWorkKeyStringSchema.pipe(DurableTable.primaryKey),
  plannerContextId: SessionIdSchema,
  status: FactoryWorkStatusSchema,
  updatedAt: Schema.String,
})
```
...
the status schema. If the app can derive UI state directly from facts plus
session snapshots, it does not need this projection at all.
...
const FactoryToolBindingsLive = FiregridToolBindings.fromAppProjections({
  ...
  executes: {
    "factory.work.record": recordFactoryWorkProjection,
...
| Product work projections | app-owned `DurableTable` schemas linked to session-plane handles, only when product UI/idempotency needs them |
...
`DarkFactoryRunSchema` should therefore not be normalized into a new Firegrid
runtime resource. The current `DarkFactoryRunSchema` name should likely become
app vocabulary such as "work item" or "work projection" if factory keeps the
row at all. Its existence is evidence that `apps/factory` is carrying
product-app state in `host.ts` because the SDK package boundaries and recipes
are not clear enough yet, not evidence that runtime needs another event
pipeline.
