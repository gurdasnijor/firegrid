...
- `firegrid-workflow-driven-runtime.PHASE_0_TARGET_REFERENCE.1`: packages/firelab contains an API-compatible clean-room reference implementation that drives the production protocol, client, edge, and channel contracts needed by the individual workflow slice while replacing only the behind-contract runtime implementation.
...
- `firegrid-workflow-driven-runtime.PHASE_0_TARGET_REFERENCE.5`: The target reference proves session create or load, start, prompt, cancel, close or resume, duplicate input identity, and workflow-owned table progress through native firelab OTel trace evidence and durable table/workflow artifacts.
