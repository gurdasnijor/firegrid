...
| **Lint ▸ knip (lint:dead)** | RED (current=21) | Unused files `events/agent-output.ts` + `tables/runtime-control-plane-time.ts` (residue → `.8`); bulk unused exports in `firelab/.../unified-kernel-validation/*` (sims → `.11/.15`); unused deps `@effect/cli`, `@firegrid/observability` in `runtime/package.json` (live). | **MIXED:** SATISFY the 2 unused deps only; **DEFER** residue files → `.8`, sim exports → `.11/.15`. Don't grind to zero now. |
...
| CI job | Runs |
|---|---|
| **Lint** | `check:specs`, `check:docs`, `lint` (eslint + `effect-native-production-cutover-check` + `runtime-public-surface-check` + `legacy-runtime-roots-scoreboard` + `runtime-target-legacy-type-only-check` + `test-layout-check` + `firelab-layout-check` + `effect:diagnostics`), `lint:dead`, `lint:dup`, `lint:deps`, `lint:effect-quality`, `lint:ast-grep` |
...
| `firelab-layout-check.mjs` | Lint | `firelab/src` top-level ⊆ {simulations,runner,experiment*,bin,index,types} | **ALIGNED** (sim methodology; green) | The coarse half of the sim shape-lock; `tf-ll90.15` adds the per-sim `{index,driver,host}` file-shape lock. Keep + `.15` extends. |
