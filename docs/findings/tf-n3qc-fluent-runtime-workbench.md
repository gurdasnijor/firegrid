...
`firelab` simulation proving the first managed-agent runtime substrate
...
- a fluent-firegrid workflow `run` activity can execute the existing
  ...
  the provider body, with the runtime import confined to the firelab
...
| Fork-backed child session was created through Durable Streams. | Trace line 15 shows fork `PUT` returned `201` with `http.request.header.stream-forked-from=/v1/stream/firelab/sessions/firelab-fluent-runtime-workbench-parent` and `http.request.header.stream-fork-offset=0000000000000000_0000000000000314`. |
...
| SandboxProvider execution is expressible as a fluent workflow activity. | Trace lines 48-49 show one `firegrid.agent_event_pipeline.source.local_process.execute` / `stream` subprocess activity span from `LocalProcessSandboxProvider`, nested under `firelab.fluent_runtime_workbench.sandbox_activity`. |
