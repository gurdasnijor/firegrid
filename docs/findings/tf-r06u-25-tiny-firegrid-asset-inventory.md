...
| `shape-c-non-recursive-start` | `index.ts`+`public-facade.ts`+`runtime.ts`; imports only `firelab` (self). Shape C deleted wholesale by #765. | **DELETE** (superseded — Shape C abandoned). |
